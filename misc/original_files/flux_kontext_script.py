...
with flux_endpoint_image.imports():
    ...
    class InferenceRequest(BaseModel):
        ...
        image_url: Optional[str] = None  # URL of the image to edit
...
class FluxService:
    ...
    def _optimize(self):
        ...
        # NOTE: Disabled para-attn for FluxKontextPipeline due to compatibility issues
        # from para_attn.first_block_cache.diffusers_adapters import apply_cache_on_pipe
        # apply_cache_on_pipe(
        #     self.pipe,
    ...
    def inference(self, request: InferenceRequest):
        ...
        if request.image_url:
            try:
                ...
                # Resize image to match requested dimensions if needed
                ...
                # Continue without input image if there's an error
                input_image = None
        ...
        # Process images without R2 storage for testing
        image_urls = []
        ...
        def process_image(image):
            ...
            buffer = BytesIO()
            pil_image.save(buffer, format=request.output_format.value, quality=request.output_quality)
            ...
            return f"test_image_{uuid.uuid4()}.{request.output_format.value.lower()}"

        # process images
        for image in images:
            image_urls.append(process_image(image))
